...
/* eslint-disable-next-line no-unused-vars */
  ...
  const handleImageClick = (work, thumbnail,post_video, item_contents) => {
    ...
    setTimeout(() => {
      ...
      // Extract last index from the URL
        const urlParts = work.item_url.split('/');
        const secondLastIndex = urlParts[urlParts.length - 2]; // Get the second last item
      // Replace spaces with dashes for the entire URL
      const url = secondLastIndex.replace(/\s+/g, "-");
     
      const queryString = `?id=${work.item_id}&title=${encodeURIComponent(url)}`;
      navigate(`/content${queryString}`, { state: { selectedImageId: work.item_id ,selectedImage:thumbnail, postVideo:post_video, itemContents: item_contents.join(',')} });
            ...
            {works.map((work) => (
              <div className="col box-gallery" key={work.id}>
                <motion.div
                  ...
                  onClick={() => handleImageClick(work, work.item_thumbnail,work.post_video, work.item_services)} // Handle zoom and redirect
